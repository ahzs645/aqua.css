...
| Menu bar (top) | `layout/menu-bar/MacTopMenuBar.tsx` (`.mac-top-menubar`, inline surface) | Menu bar | `.menu-bar`, `.menu-title`, `.menu-apple`, `.menu-dropdown`, `.menu-bar-clock` | TOKEN + FORCE | Surface color is inline (`--os-color-menubar-surface` fallback literal) → bridge sets `--os-color-menubar-surface` and forces `background-image: var(--menu-bar-pinstripe)`. Menu gradient via `--os-color-menubar-bg` → `--menu-bar-gradient`. Height: ryOS 25px vs aqua.css 28px — bridge keeps ryOS metric. |
| Menu dropdowns / menus | Radix via `menubarStyles.ts` (`MENUBAR_*_CLASS`), `ui/dropdown-menu.tsx` | Context menu | `.menu-dropdown`, `.context-menu`, `.context-menu-item`, `.aqua-menu*` | TOKEN + FORCE | Radix portals render outside the `.aqua` wrapper when scoped to a container — that's why the `aqua` class goes on `<html>`. Item hover/selection via `--os-color-selection-bg` → `--selection-focused-fill-gradient` (bridge flattens to a gradient). |
| Context menu | `ui/right-click-menu.tsx` | Context menu | `.context-menu`, `.aqua-menu` | TOKEN | Selection + panel tokens. |
| Dock | `layout/dock/MacDock.tsx` (inline surface) | Dock | `.dock`, `.dock-icon`, `.dock-separator`, `.dock-label` | TOKEN + FORCE | ryOS surface reads `--os-color-dock-surface` / `--os-color-dock-shadow` with literal fallbacks → bridge defines them from `--dock-bg` / `--dock-shadow` / `--dock-pinstripe`. Magnification/layout stays ryOS's. |
